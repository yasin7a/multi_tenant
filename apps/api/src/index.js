...
function getTenantBaseUrl(tenant) {
  if (tenant.customDomain) return `${getPublicProtocol()}://${tenant.customDomain}`
...
async function resolveHost(hostname) {
  ...
  if (host.endsWith('.localhost')) {
    return { type: 'tenant', subdomain: host.slice(0, -'.localhost'.length), isCustomDomain: false }
  }

  if (host.endsWith(`.${ROOT_DOMAIN}`)) {
    return { type: 'tenant', subdomain: host.slice(0, -(ROOT_DOMAIN.length + 1)), isCustomDomain: false }
  }
  ...
  if (tenant) return { type: 'tenant', subdomain: tenant.subdomain, isCustomDomain: true }
  ...
  return cors({
    ...
    origin(origin, cb) {
      ...
      try {
        ...
        const isDevSubdomain = url.hostname === ROOT_DOMAIN || url.hostname.endsWith(`.${ROOT_DOMAIN}`)
        const isDevPort = String(url.port || '') === String(process.env.WEB_PORT || 3000)
        if (isDevSubdomain && isDevPort && (url.protocol === 'http:' || url.protocol === 'https:')) {
          return cb(null, true)
        }
      } catch {
        // ignore
...
app.get('/internal/caddy-ask', async (req, res) => {
  ...
  if (domain === ROOT_DOMAIN || domain.endsWith(`.${ROOT_DOMAIN}`)) return res.status(403).send()
...
app.get('/auth/continue', async (req, res) => {
  ...
  const nextPath = typeof req.query.next === 'string' && req.query.next.startsWith('/') ? req.query.next : '/edit'
...
app.get('/api/profile/public', async (req, res) => {
  ...
  if (hostCtx.type !== 'tenant') return res.status(404).json({ error: 'profile not found' })
...
app.get('/api/custom-domain/verify', async (req, res) => {
  ...
  const domain = normalizeCustomDomain(req.query.domain) || authUser.tenant.customDomain
  if (!domain) return res.json({ domain: null, verified: false, status: 'none' })
  if (!isValidCustomDomain(domain)) return res.status(400).json({ error: 'invalid domain' })
...
app.post('/api/auth/register', async (req, res) => {
  ...
  const existingTenant = await prisma.tenant.findUnique({ where: { subdomain }, select: { id: true } })
  if (existingTenant) return res.status(409).json({ error: 'subdomain already taken' })
...
app.post('/api/profile', upload.single('image'), async (req, res) => {
  ...
  if (hostCtx.type === 'tenant') {
    if (hostCtx.isCustomDomain) {
      if (authUser.tenant.customDomain !== req.hostname) return res.status(403).json({ error: 'wrong tenant host' })
    ...
    return res.status(400).json({ error: 'open edit on your tenant subdomain/custom domain' })
  }
  ...
  if (!/^[a-zA-Z0-9-]+$/.test(username)) return res.status(400).json({ error: 'username can only contain letters, numbers, and hyphens' })
  ...
  if (newSubdomain !== authUser.tenant.subdomain) {
    const existingTenant = await prisma.tenant.findUnique({ where: { subdomain: newSubdomain }, select: { id: true } })
    if (existingTenant) return res.status(409).json({ error: 'subdomain already taken' })
  }

  if (customDomain && !isValidCustomDomain(customDomain)) return res.status(400).json({ error: 'invalid custom domain' })
  if (customDomain) {
    const existingDomain = await prisma.tenant.findFirst({
      where: { customDomain, NOT: { id: authUser.tenantId } },
      select: { id: true },
    })
    if (existingDomain) return res.status(409).json({ error: 'custom domain already taken' })
  }
  ...
  if (updatedUser.tenant.customDomain && req.hostname !== updatedUser.tenant.customDomain) {
